...
class WeightedPool {
    ...
    _exactTokenInForTokenOut(poolPairData, amount, exact) {
        if (exact) {
            try {
                ...
                const amt = SDK__namespace.WeightedMath._calcOutGivenIn(bnum(poolPairData.balanceIn.toString()), bnum(poolPairData.weightIn.toString()), bnum(poolPairData.balanceOut.toString()), bnum(poolPairData.weightOut.toString()), scale(amount, poolPairData.decimalsIn), bnum(poolPairData.swapFee.toString()));
    ...
    _tokenInForExactTokenOut(poolPairData, amount, exact) {
        if (exact) {
            try {
                // poolPair balances are normalised so must be scaled before use
                const amt = SDK__namespace.WeightedMath._calcInGivenOut(bnum(poolPairData.balanceIn.toString()), bnum(poolPairData.weightIn.toString()), bnum(poolPairData.balanceOut.toString()), bnum(poolPairData.weightOut.toString()), scale(amount, poolPairData.decimalsOut), bnum(poolPairData.swapFee.toString()));
                // return normalised amount
...
function _spotPriceAfterSwapExactTokenInForTokenOut$4(amount, poolPairData) {
    ...
    ans = ONE$2.div(ans.times(constants.WeiPerEther.sub(swapFee).toString()).div(constants.WeiPerEther.toString()));
    return ans;
}
// PairType = 'token->token'
// SwapType = 'swapExactOut'
...
function _spotPriceAfterSwapTokenInForExactBPTOut$2(amount, poolPairData) {
    ...
    let ans = _poolDerivativesBPT$1(amp, balances, bnum(bignumber.formatFixed(balanceOut, decimalsOut)).plus(amount), tokenIndexIn, true, true, true);
...
class StablePool {
    ...
    _exactTokenInForTokenOut(poolPairData, amount, exact) {
        try {
            ...
            const amt = SDK__namespace.StableMath._calcOutGivenIn(bnum(this.amp.toString()), poolPairData.allBalancesScaled.map((balance) => bnum(balance.toString())), poolPairData.tokenIndexIn, poolPairData.tokenIndexOut, amtScaled, bnum(poolPairData.swapFee.toString()));
            // return normalised amount
            // Using BigNumber.js decimalPlaces (dp), allows us to consider token decimal accuracy correctly,
            // i.e. when using token with 2decimals 0.002 should be returned as 0
            // Uses ROUND_DOWN mode (1)
    ...
    _tokenInForExactTokenOut(poolPairData, amount, exact) {
        try {
            ...
            // i.e. 1USDC => 1e18 not 1e6
            const amtScaled = scale(amount, 18);
            const amt = SDK__namespace.StableMath._calcInGivenOut(bnum(this.amp.toString()), poolPairData.allBalancesScaled.map((balance) => bnum(balance.toString())), poolPairData.tokenIndexIn, poolPairData.tokenIndexOut, amtScaled, bnum(poolPairData.swapFee.toString()));
            // return normalised amount
            // Using BigNumber.js decimalPlaces (dp), allows us to consider token decimal accuracy correctly,
            // i.e. when using token with 2decimals 0.002 should be returned as 0
            // Uses ROUND_UP mode (0)
...
class MetaStablePool {
    ...
    _exactTokenInForTokenOut(poolPairData, amount, exact) {
        try {
            ...
            // i.e. 1USDC => 1e18 not 1e6
            const amtScaled = scale(amount, 18);
            const amountConverted = amtScaled.times(bignumber.formatFixed(poolPairData.tokenInPriceRate, 18));
            const amt = SDK__namespace.StableMath._calcOutGivenIn(bnum(this.amp.toString()), poolPairData.allBalancesScaled.map((balance) => bnum(balance.toString())), poolPairData.tokenIndexIn, poolPairData.tokenIndexOut, amountConverted, bnum(poolPairData.swapFee.toString()));
            // return normalised amount
            // Using BigNumber.js decimalPlaces (dp), allows us to consider token decimal accuracy correctly,
            // i.e. when using token with 2decimals 0.002 should be returned as 0
            // Uses ROUND_DOWN mode (1)
            return scale(amt.div(bignumber.formatFixed(poolPairData.tokenOutPriceRate, 18)), -18).dp(poolPairData.decimalsOut, 1);
        }
        catch (err) {
            console.error(`_evmoutGivenIn: ${err.message}`);
            return ZERO$1;
    ...
    _tokenInForExactTokenOut(poolPairData, amount, exact) {
        try {
            ...
            // i.e. 1USDC => 1e18 not 1e6
            const amtScaled = scale(amount, 18);
            const amountConverted = amtScaled.times(bignumber.formatFixed(poolPairData.tokenOutPriceRate, 18));
            const amt = SDK__namespace.StableMath._calcInGivenOut(bnum(this.amp.toString()), poolPairData.allBalancesScaled.map((balance) => bnum(balance.toString())), poolPairData.tokenIndexIn, poolPairData.tokenIndexOut, amountConverted, bnum(poolPairData.swapFee.toString()));
            // return normalised amount
            // Using BigNumber.js decimalPlaces (dp), allows us to consider token decimal accuracy correctly,
            // i.e. when using token with 2decimals 0.002 should be returned as 0
            // Uses ROUND_UP mode (0)
            return scale(amt.div(bignumber.formatFixed(poolPairData.tokenInPriceRate, 18)), -18).dp(poolPairData.decimalsIn, 0);
        }
        catch (err) {
            console.error(`_evminGivenOut: ${err.message}`);
            return ZERO$1;
...
function _exactMainTokenInForBPTOut(amount, poolPairData) {
    ...
    const mainBalance = bnum(bignumber.formatFixed(poolPairData.balanceIn, poolPairData.decimalsIn));
    const wrappedBalance = bnum(bignumber.formatFixed(poolPairData.wrappedBalance.toString(), poolPairData.wrappedDecimals));
    const virtualBptSupply = bnum(bignumber.formatFixed(poolPairData.virtualBptSupply, 18));
    const params = [
...
function _BPTInForExactMainTokenOut(amount, poolPairData) {
    ...
    const mainBalance = bnum(bignumber.formatFixed(poolPairData.balanceOut, poolPairData.decimalsOut));
    const wrappedBalance = bnum(bignumber.formatFixed(poolPairData.wrappedBalance.toString(), poolPairData.wrappedDecimals));
    const virtualBptSupply = bnum(bignumber.formatFixed(poolPairData.virtualBptSupply, 18));
    const params = [
        bnum(bignumber.formatFixed(poolPairData.swapFee, 18)),
...
function _exactBPTInForTokenOut(amount, poolPairData) {
    ...
    **********************************************************************************************/
    ...
    const currentInvariant = _invariant(amp, balances);
    // Calculate new invariant
    const bnumBalanceIn = MAX_TOKEN_BALANCE.minus(bnum(bignumber.formatFixed(balanceIn, decimalsIn)));
...
class PhantomStablePool {
    ...
    _exactTokenInForTokenOut(poolPairData, amount, exact) {
        try {
            ...
            const amtWithFee = this.subtractSwapFeeAmount(bignumber.BigNumber.from(amtScaled.toString()), poolPairData.swapFee);
            ...
            if (poolPairData.pairType === PairTypes.TokenToBpt) {
                ...
                amt = SDK__namespace.StableMath._calcBptOutGivenExactTokensIn(bnum(this.amp.toString()), poolPairData.allBalancesScaled.map((b) => bnum(b.toString())), amountsIn, bnum(poolPairData.virtualBptSupply.toString()), ZERO$1 // Fee is handled above
                );
            }
            else if (poolPairData.pairType === PairTypes.BptToToken) {
                amt = SDK__namespace.StableMath._calcTokenOutGivenExactBptIn(bnum(this.amp.toString()), poolPairData.allBalancesScaled.map((b) => bnum(b.toString())), poolPairData.tokenIndexOut, bnum(amountConverted.toString()), bnum(poolPairData.virtualBptSupply.toString()), ZERO$1 // Fee is handled above
                );
            }
            else {
                amt = SDK__namespace.StableMath._calcOutGivenIn(bnum(this.amp.toString()), poolPairData.allBalancesScaled.map((b) => bnum(b.toString())), poolPairData.tokenIndexIn, poolPairData.tokenIndexOut, bnum(amountConverted.toString()), ZERO$1 // Fee is handled above
                );
            }
            // return normalised amount
            // Using BigNumber.js decimalPlaces (dp), allows us to consider token decimal accuracy correctly,
    ...
    _tokenInForExactTokenOut(poolPairData, amount, exact) {
        try {
            ...
            const amountConverted = amtScaled
                .times(bignumber.formatFixed(poolPairData.tokenOutPriceRate, 18))
                .dp(0);
            let amt;
            if (poolPairData.pairType === PairTypes.TokenToBpt) {
                amt = SDK__namespace.StableMath._calcTokenInGivenExactBptOut(bnum(this.amp.toString()), poolPairData.allBalancesScaled.map((b) => bnum(b.toString())), poolPairData.tokenIndexIn, bnum(amountConverted.toString()), bnum(poolPairData.virtualBptSupply.toString()), ZERO$1 // Fee is handled above
                );
            }
            else if (poolPairData.pairType === PairTypes.BptToToken) {
                ...
                amt = SDK__namespace.StableMath._calcBptInGivenExactTokensOut(bnum(this.amp.toString()), poolPairData.allBalancesScaled.map((b) => bnum(b.toString())), amountsOut, bnum(poolPairData.virtualBptSupply.toString()), ZERO$1 // Fee is handled above
                );
            }
            else {
                amt = SDK__namespace.StableMath._calcInGivenOut(bnum(this.amp.toString()), poolPairData.allBalancesScaled.map((b) => bnum(b.toString())), poolPairData.tokenIndexIn, poolPairData.tokenIndexOut, amountConverted, ZERO$1 // Fee is handled above
                );
            }
            const returnScaled = amt
                ...
                .dp(0);
            // In Phantom Pools every time there is a swap (token per token, bpt per token or token per bpt), we substract the fee from the amount in
            const returnWithFee = bnum(this.addSwapFeeAmount(bignumber.BigNumber.from(returnScaled.toString()), poolPairData.swapFee).toString());
            // return human number
...
function EVMgetOutputAmountSwap(pool, poolPairData, swapType, amount) {
    ...
    if (swapType === exports.SwapTypes.SwapExactIn) {
        ...
        if (pool.poolType === exports.PoolTypes.Weighted ||
    ...
    pool.updateTokenBalanceForPool(tokenIn, balanceIn.add(bignumber.parseFixed(returnAmount.dp(poolPairData.decimalsIn).toString(), poolPairData.decimalsIn)));
    pool.updateTokenBalanceForPool(tokenOut, balanceOut.sub(bignumber.parseFixed(amount.dp(poolPairData.decimalsOut).toString(), poolPairData.decimalsOut)));
    return returnAmount;
}
...
function filterHopPools(tokenIn, tokenOut, hopTokens, poolsOfInterest) {
    ...
    if (hopTokens.length === 0) {
        for (const id in poolsOfInterest) {
            if (poolsOfInterest[id].swapPairType !== exports.SwapPairType.Direct) {
                continue;
            }
...
function getLinearStaBal3Paths(tokenIn, tokenOut, poolsAllDict, poolsFilteredDict, config) {
    ...
    // If both tokenIn and tokenOut belong to linear pools
        ...
        const staBal3TokenOutPath = getMostLiquidPath([staBal3Pool.address, tokenOut], 0, [], poolsFilteredDict, exports.SwapPairType.HopOut);
        ...
        if (!wethStaBal3Pool)
            return pathsUsingLinear;
        const wethTokenOutPath = getMostLiquidPath([staBal3Pool.address, config.weth, tokenOut], 1, [wethStaBal3Pool], poolsFilteredDict, exports.SwapPairType.HopOut);
        ...
        const tokenInStaBal3Path = getMostLiquidPath([tokenIn, staBal3Pool.address], 0, [], poolsFilteredDict, exports.SwapPairType.HopIn);
        ...
        if (!wethStaBal3Pool)
            return pathsUsingLinear;
        const tokenInWethPath = getMostLiquidPath([tokenIn, config.weth, staBal3Pool.address], 0, [wethStaBal3Pool], poolsFilteredDict, exports.SwapPairType.HopIn);
...
function getPathsUsingStaBalPool(tokenIn, tokenOut, poolsAll, poolsFiltered, config) {
    ...
    const metastablePoolIdIn = getHighestLiquidityPool(tokenIn, hopTokenStaBal, exports.SwapPairType.HopIn, poolsFiltered);
    // Finds the best metastable Pool with tokenOut/staBal3Bpt or returns null if doesn't exist
    const metastablePoolIdOut = getHighestLiquidityPool(hopTokenStaBal, tokenOut, exports.SwapPairType.HopOut, poolsFiltered);
    if (metastablePoolIdIn && !metastablePoolIdOut) {
        ...
        const mostLiquidLastPool = getHighestLiquidityPool(usdcConnectingPoolInfo.usdc, tokenOut, exports.SwapPairType.HopOut, poolsFiltered);
    ...
    if (!metastablePoolIdIn && metastablePoolIdOut) {
        ...
        const mostLiquidFirstPool = getHighestLiquidityPool(tokenIn, usdcConnectingPoolInfo.usdc, exports.SwapPairType.HopIn, poolsFiltered);
...
class SwapCostCalculator {
     ...
     */
    convertGasCostToToken(tokenAddress, tokenDecimals, gasPriceWei, swapGas = bignumber.BigNumber.from('35000')) {
        return __awaiter(this, void 0, void 0, function* () {
