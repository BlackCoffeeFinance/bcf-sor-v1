import { formatFixed, parseFixed, BigNumber as BigNumber$1 } from '@ethersproject/bignumber';
...
function _spotPriceAfterSwapTokenInForExactBPTOut$3(amount, poolPairData) {
    ...
    const f = parseFloat(formatFixed(poolPairData.swapFee, 18));
    return bnum((Math.pow(((Aobpt + Bbpt) / Bbpt), (1 / wi)) * Bi) /
...
function _spotPriceAfterSwapTokenInForExactTokenOut$5(amount, poolPairData) {
    ...
    const Ao = amount.toNumber();
    const f = parseFloat(formatFixed(poolPairData.swapFee, 18));
    return bnum(-((Bi * Math.pow((Bo / (-Ao + Bo)), ((wi + wo) / wi)) * wo) /
        (Bo * (-1 + f) * wi)));
}
/////////
...
function _derivativeSpotPriceAfterSwapExactTokenInForTokenOut$5(amount, poolPairData) {
    ...
    const Ai = amount.toNumber();
    const f = parseFloat(formatFixed(poolPairData.swapFee, 18));
    return bnum((wi + wo) / (Bo * Math.pow((Bi / (Ai + Bi - Ai * f)), (wi / wo)) * wi));
}
// PairType = 'token->token'
// SwapType = 'swapExactOut'
function _derivativeSpotPriceAfterSwapTokenInForExactTokenOut$5(amount, poolPairData) {
    ...
    const Ao = amount.toNumber();
    const f = parseFloat(formatFixed(poolPairData.swapFee, 18));
    return bnum(-((Bi * Math.pow((Bo / (-Ao + Bo)), (wo / wi)) * wo * (wi + wo)) /
...
class WeightedPool {
    ...
    static fromPool(pool) {
        ...
        return new WeightedPool(pool.id, pool.address, pool.swapFee, pool.totalWeight, pool.totalShares, pool.tokens, pool.tokensList);
    }
    setTypeForSwap(type) {
    ...
    getNormalizedLiquidity(poolPairData) {
        return bnum(formatFixed(poolPairData.balanceOut
    ...
    getLimitAmountSwap(poolPairData, swapType) {
        if (swapType === SwapTypes.SwapExactIn) {
            return bnum(formatFixed(poolPairData.balanceIn.mul(this.MAX_IN_RATIO).div(WeiPerEther), poolPairData.decimalsIn));
        }
        else {
            return bnum(formatFixed(poolPairData.balanceOut.mul(this.MAX_OUT_RATIO).div(WeiPerEther), poolPairData.decimalsOut));
        }
    }
    ...
    _exactTokenInForTokenOut(poolPairData, amount, exact) {
        if (exact) {
            try {
                ...
                const amt = SDK.WeightedMath._calcOutGivenIn(bnum(poolPairData.balanceIn.toString()), bnum(poolPairData.weightIn.toString()), bnum(poolPairData.balanceOut.toString()), bnum(poolPairData.weightOut.toString()), scale(amount, poolPairData.decimalsIn), bnum(poolPairData.swapFee.toString()));
        ...
        return _exactTokenInForTokenOut$4(amount, poolPairData).dp(poolPairData.decimalsOut, 1);
    }
    // Using BigNumber.js decimalPlaces (dp), allows us to consider token decimal accuracy correctly,
    // i.e. when using token with 2decimals 0.002 should be returned as 0
    ...
    _tokenInForExactTokenOut(poolPairData, amount, exact) {
        if (exact) {
            try {
                // poolPair balances are normalised so must be scaled before use
                const amt = SDK.WeightedMath._calcInGivenOut(bnum(poolPairData.balanceIn.toString()), bnum(poolPairData.weightIn.toString()), bnum(poolPairData.balanceOut.toString()), bnum(poolPairData.weightOut.toString()), scale(amount, poolPairData.decimalsOut), bnum(poolPairData.swapFee.toString()));
                // return normalised amount
        ...
        return _tokenInForExactTokenOut$4(amount, poolPairData).dp(poolPairData.decimalsIn, 0);
    }
    _spotPriceAfterSwapExactTokenInForTokenOut(poolPairData, amount) {
        return _spotPriceAfterSwapExactTokenInForTokenOut$5(amount, poolPairData);
    }
    _spotPriceAfterSwapTokenInForExactTokenOut(poolPairData, amount) {
        return _spotPriceAfterSwapTokenInForExactTokenOut$5(amount, poolPairData);
    }
...
function _tokenInForExactBPTOut$1(amount, poolPairData) {
    ...
    **********************************************************************************************/
    ...
    // return amountIn
    return amountInAfterFee.div(ONE$2.minus(tokenBalancePercentageExcess
        .times(swapFee.toString())
        .div(WeiPerEther.toString())));
}
//This function calculates the balance of a given token (tokenIndex)
// given all the other balances and the invariant
function _getTokenBalanceGivenInvariantAndAllOtherBalances$1(amp, balances, inv, tokenIndex) {
    let p = inv;
    let sum = ZERO$1;
    const totalCoins = balances.length;
    let nPowN = ONE$2;
...
/// SpotPriceAfterSwap
/////////
function _poolDerivativesBPT$1(amp, balances, bptSupply, tokenIndexIn, is_first_derivative, is_BPT_out, wrt_out) {
    const totalCoins = balances.length;
    const D = _invariant$2(amp, balances);
    let S = ZERO$1;
...
function _spotPriceAfterSwapExactTokenInForTokenOut$4(amount, poolPairData) {
    const { amp, allBalances, tokenIndexIn, tokenIndexOut, swapFee } = poolPairData;
    const balances = [...allBalances];
    balances[tokenIndexIn] = balances[tokenIndexIn].plus(amount.times(WeiPerEther.sub(swapFee).toString()).div(WeiPerEther.toString()));
    balances[tokenIndexOut] = balances[tokenIndexOut].minus(_exactTokenInForTokenOut$3(amount, poolPairData));
    let ans = _poolDerivatives$2(amp, balances, tokenIndexIn, tokenIndexOut, true, false);
    ans = ONE$2.div(ans.times(WeiPerEther.sub(swapFee).toString()).div(WeiPerEther.toString()));
    return ans;
}
// PairType = 'token->token'
// SwapType = 'swapExactOut'
...
function _feeFactor$1(balances, tokenIndex, swapFee) {
    ...
    const tokenBalancePercentageExcess = ONE$2.minus(currentWeight);
    return ONE$2.minus(tokenBalancePercentageExcess
        .times(swapFee.toString())
        .div(WeiPerEther.toString()));
}
// PairType = 'token->BPT'
...
function _spotPriceAfterSwapTokenInForExactBPTOut$2(amount, poolPairData) {
    ...
    let ans = _poolDerivativesBPT$1(amp, balances, bnum(formatFixed(balanceOut, decimalsOut)).plus(amount), tokenIndexIn, true, true, true);
...
class StablePool {
    ...
    parsePoolPairData(tokenIn, tokenOut) {
        ...
        const allBalancesScaled = this.tokens.map(({ balance }) => parseFixed(balance, 18));
    ...
    getNormalizedLiquidity(poolPairData) {
        ...
        return bnum(formatFixed(poolPairData.balanceOut.mul(poolPairData.amp), poolPairData.decimalsOut + StablePool.AMP_DECIMALS));
    }
    getLimitAmountSwap(poolPairData, swapType) {
    ...
    _exactTokenInForTokenOut(poolPairData, amount, exact) {
        try {
            ...
            const amt = SDK.StableMath._calcOutGivenIn(bnum(this.amp.toString()), poolPairData.allBalancesScaled.map((balance) => bnum(balance.toString())), poolPairData.tokenIndexIn, poolPairData.tokenIndexOut, amtScaled, bnum(poolPairData.swapFee.toString()));
            // return normalised amount
            // Using BigNumber.js decimalPlaces (dp), allows us to consider token decimal accuracy correctly,
            // i.e. when using token with 2decimals 0.002 should be returned as 0
            // Uses ROUND_DOWN mode (1)
    ...
    _tokenInForExactTokenOut(poolPairData, amount, exact) {
        try {
            ...
            // i.e. 1USDC => 1e18 not 1e6
            const amtScaled = scale(amount, 18);
            const amt = SDK.StableMath._calcInGivenOut(bnum(this.amp.toString()), poolPairData.allBalancesScaled.map((balance) => bnum(balance.toString())), poolPairData.tokenIndexIn, poolPairData.tokenIndexOut, amtScaled, bnum(poolPairData.swapFee.toString()));
            // return normalised amount
            // Using BigNumber.js decimalPlaces (dp), allows us to consider token decimal accuracy correctly,
            // i.e. when using token with 2decimals 0.002 should be returned as 0
            // Uses ROUND_UP mode (0)
...
function _derivativeSpotPriceAfterSwapExactTokenInForTokenOut$3(amount, poolPairData) {
    ...
    balances[tokenIndexIn] = balances[tokenIndexIn].plus(amount.times(WeiPerEther.sub(swapFee).toString()).div(WeiPerEther.toString()));
    balances[tokenIndexOut] = balances[tokenIndexOut].minus(_exactTokenInForTokenOut$2(amount, poolPairData));
    return _poolDerivatives$1(amp, balances, tokenIndexIn, tokenIndexOut, false, false);
}
// PairType = 'token->token'
// SwapType = 'swapExactOut'
function _derivativeSpotPriceAfterSwapTokenInForExactTokenOut$3(amount, poolPairData) {
    ...
    balances[tokenIndexOut] = balances[tokenIndexOut].minus(amount);
    const feeFactor = WeiPerEther.div(swapFee).toString();
    return _poolDerivatives$1(amp, balances, tokenIndexIn, tokenIndexOut, false, true).div(feeFactor);
}
...
class MetaStablePool {
    ...
    parsePoolPairData(tokenIn, tokenOut) {
        ...
        const balanceIn = formatFixed(parseFixed(tI.balance, decimalsIn).mul(tokenInPriceRate).div(WeiPerEther), decimalsIn);
        const tokenIndexOut = this.tokens.findIndex((t) => getAddress(t.address) === getAddress(tokenOut));
        if (tokenIndexOut < 0)
            throw 'Pool does not contain tokenOut';
        const tO = this.tokens[tokenIndexOut];
        ...
        const balanceOut = formatFixed(parseFixed(tO.balance, decimalsOut).mul(tokenOutPriceRate).div(WeiPerEther), decimalsOut);
        // Get all token balances
        const allBalances = this.tokens.map(({ balance, priceRate }) => bnum(balance).times(priceRate));
        const allBalancesScaled = this.tokens.map(({ balance, priceRate }) => parseFixed(balance, 18).mul(parseFixed(priceRate, 18)).div(WeiPerEther));
    ...
    getNormalizedLiquidity(poolPairData) {
        // This is an approximation as the actual normalized liquidity is a lot more complicated to calculate
        return bnum(formatFixed(poolPairData.balanceOut.mul(poolPairData.amp), poolPairData.decimalsOut + MetaStablePool.AMP_DECIMALS));
    }
    getLimitAmountSwap(poolPairData, swapType) {
        // We multiply ratios by 10**-18 because we are in normalized space
        // so 0.5 should be 0.5 and not 500000000000000000
        ...
        if (swapType === SwapTypes.SwapExactIn) {
            return bnum(formatFixed(poolPairData.balanceIn
    ...
    _exactTokenInForTokenOut(poolPairData, amount, exact) {
        try {
            ...
            // i.e. 1USDC => 1e18 not 1e6
            const amtScaled = scale(amount, 18);
            const amountConverted = amtScaled.times(formatFixed(poolPairData.tokenInPriceRate, 18));
            const amt = SDK.StableMath._calcOutGivenIn(bnum(this.amp.toString()), poolPairData.allBalancesScaled.map((balance) => bnum(balance.toString())), poolPairData.tokenIndexIn, poolPairData.tokenIndexOut, amountConverted, bnum(poolPairData.swapFee.toString()));
            // return normalised amount
            // Using BigNumber.js decimalPlaces (dp), allows us to consider token decimal accuracy correctly,
            // i.e. when using token with 2decimals 0.002 should be returned as 0
            // Uses ROUND_DOWN mode (1)
            return scale(amt.div(formatFixed(poolPairData.tokenOutPriceRate, 18)), -18).dp(poolPairData.decimalsOut, 1);
        }
        catch (err) {
            console.error(`_evmoutGivenIn: ${err.message}`);
            return ZERO$1;
    ...
    _tokenInForExactTokenOut(poolPairData, amount, exact) {
        try {
            ...
            // i.e. 1USDC => 1e18 not 1e6
            const amtScaled = scale(amount, 18);
            const amountConverted = amtScaled.times(formatFixed(poolPairData.tokenOutPriceRate, 18));
            const amt = SDK.StableMath._calcInGivenOut(bnum(this.amp.toString()), poolPairData.allBalancesScaled.map((balance) => bnum(balance.toString())), poolPairData.tokenIndexIn, poolPairData.tokenIndexOut, amountConverted, bnum(poolPairData.swapFee.toString()));
            // return normalised amount
            // Using BigNumber.js decimalPlaces (dp), allows us to consider token decimal accuracy correctly,
            // i.e. when using token with 2decimals 0.002 should be returned as 0
            // Uses ROUND_UP mode (0)
            return scale(amt.div(formatFixed(poolPairData.tokenInPriceRate, 18)), -18).dp(poolPairData.decimalsIn, 0);
        }
        catch (err) {
            console.error(`_evminGivenOut: ${err.message}`);
            return ZERO$1;
    ...
    _derivativeSpotPriceAfterSwapExactTokenInForTokenOut(poolPairData, amount) {
        const priceRateOut = formatFixed(poolPairData.tokenOutPriceRate, 18);
        return _derivativeSpotPriceAfterSwapExactTokenInForTokenOut$3(amount, poolPairData).times(priceRateOut);
    }
    _derivativeSpotPriceAfterSwapTokenInForExactTokenOut(poolPairData, amount) {
        const priceRateIn = formatFixed(poolPairData.tokenInPriceRate, 18);
        const priceRateOut = formatFixed(poolPairData.tokenOutPriceRate, 18);
...
const _calcBptOutPerWrappedIn = (wrappedIn, mainBalance, wrappedBalance, bptSupply, params) => {
    ...
    const newBptBalance = divDown(mul(bptSupply, newInvariant), previousInvariant);
...
const _calcWrappedInPerBptOut = (bptOut, mainBalance, wrappedBalance, bptSupply, params) => {
    ...
    const newWrappedBalance = sub$1(divUp(mul(newBptBalance, previousInvariant), bptSupply), nominalMain);
...
const _fromNominal = (nominal, params) => {
    ...
    if (nominal.lt(params.lowerTarget)) {
        return divDown$1(add(nominal, mulDown(params.fee, params.lowerTarget)), add(ONE$1, params.fee));
    }
    else if (nominal.lte(params.upperTarget)) {
        return nominal;
    }
    else {
        return divDown$1(sub$1(nominal, mulDown(params.fee, params.upperTarget)), sub$1(ONE$1, params.fee));
    }
};
...
function _exactMainTokenInForBPTOut(amount, poolPairData) {
    ...
    const mainBalance = bnum(formatFixed(poolPairData.balanceIn, poolPairData.decimalsIn));
    const wrappedBalance = bnum(formatFixed(poolPairData.wrappedBalance.toString(), poolPairData.wrappedDecimals));
    const virtualBptSupply = bnum(formatFixed(poolPairData.virtualBptSupply, 18));
    const params = [
...
function _BPTInForExactMainTokenOut(amount, poolPairData) {
    ...
    const mainBalance = bnum(formatFixed(poolPairData.balanceOut, poolPairData.decimalsOut));
    const wrappedBalance = bnum(formatFixed(poolPairData.wrappedBalance.toString(), poolPairData.wrappedDecimals));
    const virtualBptSupply = bnum(formatFixed(poolPairData.virtualBptSupply, 18));
    const params = [
        bnum(formatFixed(poolPairData.swapFee, 18)),
...
function _spotPriceAfterSwapTokenInForExactBPTOut$1(amount, poolPairData) {
    ...
    const afterNominalMain = previousNominalMain.plus(deltaNominalMain);
    return poolFactor.times(rightDerivativeFromNominal(afterNominalMain, params));
}
// PairType = 'BPT->token'
// SwapType = 'swapExactIn'
function _spotPriceAfterSwapExactBPTInForTokenOut$1(amount, poolPairData) {
    ...
    const afterNominalMain = previousNominalMain.minus(deltaNominalMain);
    return bnum(1).div(poolFactor.times(leftDerivativeFromNominal(afterNominalMain, params)));
}
// PairType = 'BPT->token'
// SwapType = 'swapExactOut'
...
class LinearPool {
    ...
    getLimitAmountSwap(poolPairData, swapType) {
        ...
        if (swapType === SwapTypes.SwapExactIn) {
            if (linearPoolPairData.pairType === PairTypes$1.MainTokenToBpt) {
                return _mainTokenInForExactBPTOut(balanceOutHuman, linearPoolPairData)
                ...
                const limit = _BPTInForExactMainTokenOut(balanceOutHuman, linearPoolPairData)
                    .times(bnum(this.ALMOST_ONE.toString()))
                    .div(bnum(WeiPerEther.toString()));
                return limit;
            }
                ...
                // Limit is amount of BPT in for pool balance of tokenOut
                const limit = _calcBptInPerWrappedOut(bnum(poolPairData.balanceOut.toString()), bnum(linearPoolPairData.mainBalanceScaled.toString()), bnum(linearPoolPairData.wrappedBalanceScaled.toString()), bnum(linearPoolPairData.bptBalanceScaled.toString()), {
                    fee: bnum(poolPairData.swapFee.toString()),
                    lowerTarget: bnum(linearPoolPairData.lowerTarget.toString()),
                    upperTarget: bnum(linearPoolPairData.upperTarget.toString()),
                })
                    .times(bnum(this.ALMOST_ONE.toString()))
                    .div(bnum(WeiPerEther.toString()))
                    .div(bnum(WeiPerEther.toString()));
    ...
    _exactTokenInForTokenOut(poolPairData, amount, exact) {
        if (poolPairData.pairType === PairTypes$1.MainTokenToBpt) {
            ...
            return this._exactWrappedTokenInForBPTOut(poolPairData, amount, exact);
        }
        else if (poolPairData.pairType === PairTypes$1.BptToWrappedToken) {
            return this._exactBPTInForWrappedTokenOut(poolPairData, amount, exact);
        }
        else if (poolPairData.pairType === PairTypes$1.MainTokenToWrappedToken) {
            return this._exactMainTokenInForWrappedOut(poolPairData, amount, exact);
        }
        else if (poolPairData.pairType === PairTypes$1.WrappedTokenToMainToken) {
            return this._exactWrappedTokenInForMainOut(poolPairData, amount, exact);
        }
        else
            return bnum(0);
    }
    _exactWrappedTokenInForMainOut(poolPairData, amount, exact) {
        try {
            ...
            // i.e. 1USDC => 1e18 not 1e6
            const amtScaled = scale(amount, 18);
            const amt = _calcMainOutPerWrappedIn(amtScaled, bnum(poolPairData.mainBalanceScaled.toString()), {
                fee: bnum(poolPairData.swapFee.toString()),
    ...
    _exactMainTokenInForWrappedOut(poolPairData, amount, exact) {
        try {
            ...
            // i.e. 1USDC => 1e18 not 1e6
            const amtScaled = scale(amount, 18);
            const amt = _calcWrappedOutPerMainIn(amtScaled, bnum(poolPairData.mainBalanceScaled.toString()), {
                fee: bnum(poolPairData.swapFee.toString()),
                lowerTarget: bnum(poolPairData.lowerTarget.toString()),
                upperTarget: bnum(poolPairData.upperTarget.toString()),
            });
    ...
    _exactMainTokenInForBPTOut(poolPairData, amount, exact) {
        if (exact) {
            try {
                ...
                // i.e. 1USDC => 1e18 not 1e6
                const amtScaled = scale(amount, 18);
                const amt = _calcBptOutPerMainIn(amtScaled, bnum(poolPairData.mainBalanceScaled.toString()), bnum(poolPairData.wrappedBalanceScaled.toString()), bnum(poolPairData.virtualBptSupply.toString()), {
                    fee: bnum(poolPairData.swapFee.toString()),
                    lowerTarget: bnum(poolPairData.lowerTarget.toString()),
                    upperTarget: bnum(poolPairData.upperTarget.toString()),
                });
    ...
    _exactBPTInForMainTokenOut(poolPairData, amount, exact) {
        if (exact) {
            try {
                ...
                // i.e. 1USDC => 1e18 not 1e6
                const amtScaled = scale(amount, 18);
                const amt = _calcMainOutPerBptIn(amtScaled, bnum(poolPairData.mainBalanceScaled.toString()), bnum(poolPairData.wrappedBalanceScaled.toString()), bnum(poolPairData.virtualBptSupply.toString()), {
                    fee: bnum(poolPairData.swapFee.toString()),
                    lowerTarget: bnum(poolPairData.lowerTarget.toString()),
                    upperTarget: bnum(poolPairData.upperTarget.toString()),
                });
            ...
            return _exactBPTInForMainTokenOut(amount, poolPairData);
    }
    _exactWrappedTokenInForBPTOut(poolPairData, amount, exact) {
        try {
            ...
            // i.e. 1USDC => 1e18 not 1e6
            const amtScaled = scale(amount, 18);
            const amt = _calcBptOutPerWrappedIn(amtScaled, bnum(poolPairData.mainBalanceScaled.toString()), bnum(poolPairData.wrappedBalanceScaled.toString()), bnum(poolPairData.virtualBptSupply.toString()), {
                fee: bnum(poolPairData.swapFee.toString()),
                lowerTarget: bnum(poolPairData.lowerTarget.toString()),
                upperTarget: bnum(poolPairData.upperTarget.toString()),
            });
    ...
    _exactBPTInForWrappedTokenOut(poolPairData, amount, exact) {
        try {
            ...
            // i.e. 1USDC => 1e18 not 1e6
            const amtScaled = scale(amount, 18);
            const amt = _calcWrappedOutPerBptIn(amtScaled, bnum(poolPairData.mainBalanceScaled.toString()), bnum(poolPairData.wrappedBalanceScaled.toString()), bnum(poolPairData.virtualBptSupply.toString()), {
                fee: bnum(poolPairData.swapFee.toString()),
                lowerTarget: bnum(poolPairData.lowerTarget.toString()),
                upperTarget: bnum(poolPairData.upperTarget.toString()),
            });
    ...
    _tokenInForExactTokenOut(poolPairData, amount, exact) {
        if (poolPairData.pairType === PairTypes$1.MainTokenToBpt) {
            ...
            return this._wrappedTokenInForExactBPTOut(poolPairData, amount, exact);
        }
        else if (poolPairData.pairType === PairTypes$1.BptToWrappedToken) {
            return this._BPTInForExactWrappedTokenOut(poolPairData, amount, exact);
        }
        else if (poolPairData.pairType === PairTypes$1.MainTokenToWrappedToken) {
            return this._mainTokenInForExactWrappedOut(poolPairData, amount, exact);
        }
        else if (poolPairData.pairType === PairTypes$1.WrappedTokenToMainToken) {
            return this._wrappedTokenInForExactMainOut(poolPairData, amount, exact);
        }
        else
    ...
    _wrappedTokenInForExactMainOut(poolPairData, amount, exact) {
        try {
            ...
            // i.e. 1USDC => 1e18 not 1e6
            const amtScaled = scale(amount, 18);
            const amt = _calcWrappedInPerMainOut(amtScaled, bnum(poolPairData.mainBalanceScaled.toString()), {
                fee: bnum(poolPairData.swapFee.toString()),
                lowerTarget: bnum(poolPairData.lowerTarget.toString()),
                upperTarget: bnum(poolPairData.upperTarget.toString()),
            });
    ...
    _mainTokenInForExactWrappedOut(poolPairData, amount, exact) {
        try {
            ...
            // i.e. 1USDC => 1e18 not 1e6
            const amtScaled = scale(amount, 18);
            const amt = _calcMainInPerWrappedOut(amtScaled, bnum(poolPairData.mainBalanceScaled.toString()), {
                fee: bnum(poolPairData.swapFee.toString()),
                lowerTarget: bnum(poolPairData.lowerTarget.toString()),
                upperTarget: bnum(poolPairData.upperTarget.toString()),
            });
    ...
    _mainTokenInForExactBPTOut(poolPairData, amount, exact) {
        if (exact) {
            try {
                ...
                // in = main
                // out = BPT
                const amt = _calcMainInPerBptOut(amtScaled, bnum(poolPairData.mainBalanceScaled.toString()), bnum(poolPairData.wrappedBalanceScaled.toString()), bnum(poolPairData.virtualBptSupply.toString()), {
                    fee: bnum(poolPairData.swapFee.toString()),
                    lowerTarget: bnum(poolPairData.lowerTarget.toString()),
                    upperTarget: bnum(poolPairData.upperTarget.toString()),
                });
    ...
    _BPTInForExactMainTokenOut(poolPairData, amount, exact) {
        if (exact) {
            try {
                ...
                // i.e. 1USDC => 1e18 not 1e6
                const amtScaled = scale(amount, 18);
                const amt = _calcBptInPerMainOut(amtScaled, bnum(poolPairData.mainBalanceScaled.toString()), bnum(poolPairData.wrappedBalanceScaled.toString()), bnum(poolPairData.virtualBptSupply.toString()), {
                    fee: bnum(poolPairData.swapFee.toString()),
                    lowerTarget: bnum(poolPairData.lowerTarget.toString()),
                    upperTarget: bnum(poolPairData.upperTarget.toString()),
                });
    ...
    _wrappedTokenInForExactBPTOut(poolPairData, amount, exact) {
        try {
            ...
            // i.e. 1USDC => 1e18 not 1e6
            const amtScaled = scale(amount, 18);
            const amt = _calcWrappedInPerBptOut(amtScaled, bnum(poolPairData.mainBalanceScaled.toString()), bnum(poolPairData.wrappedBalanceScaled.toString()), bnum(poolPairData.virtualBptSupply.toString()), {
                fee: bnum(poolPairData.swapFee.toString()),
                lowerTarget: bnum(poolPairData.lowerTarget.toString()),
                upperTarget: bnum(poolPairData.upperTarget.toString()),
            });
    ...
    _BPTInForExactWrappedTokenOut(poolPairData, amount, exact) {
        try {
            ...
            // i.e. 1USDC => 1e18 not 1e6
            const amtScaled = scale(amount, 18);
            const amt = _calcBptInPerWrappedOut(amtScaled, bnum(poolPairData.mainBalanceScaled.toString()), bnum(poolPairData.wrappedBalanceScaled.toString()), bnum(poolPairData.virtualBptSupply.toString()), {
                fee: bnum(poolPairData.swapFee.toString()),
                lowerTarget: bnum(poolPairData.lowerTarget.toString()),
                upperTarget: bnum(poolPairData.upperTarget.toString()),
            });
    ...
    _spotPriceAfterSwapExactTokenInForTokenOut(poolPairData, amount) {
        ...
        if (poolPairData.pairType === PairTypes$1.MainTokenToBpt ||
            ...
            return this._spotPriceAfterSwapExactTokenInForBPTOut(poolPairData, amount);
        }
        else if (poolPairData.pairType === PairTypes$1.BptToMainToken ||
            poolPairData.pairType === PairTypes$1.BptToWrappedToken) {
            return this._spotPriceAfterSwapExactBPTInForTokenOut(poolPairData, amount);
        }
        else
    ...
    _spotPriceAfterSwapTokenInForExactTokenOut(poolPairData, amount) {
        ...
        if (poolPairData.pairType === PairTypes$1.MainTokenToBpt ||
            poolPairData.pairType === PairTypes$1.WrappedTokenToBpt) {
            return this._spotPriceAfterSwapTokenInForExactBPTOut(poolPairData, amount);
        }
        else if (poolPairData.pairType === PairTypes$1.BptToMainToken ||
            poolPairData.pairType === PairTypes$1.BptToWrappedToken) {
            return this._spotPriceAfterSwapBPTInForExactTokenOut(poolPairData, amount);
        }
        else
    ...
    _derivativeSpotPriceAfterSwapExactTokenInForTokenOut(poolPairData, amount) {
        if (poolPairData.pairType === PairTypes$1.MainTokenToBpt) {
            return this._derivativeSpotPriceAfterSwapExactTokenInForBPTOut(poolPairData, amount);
        }
        else if (poolPairData.pairType === PairTypes$1.BptToMainToken) {
            return this._derivativeSpotPriceAfterSwapExactBPTInForTokenOut(poolPairData, amount);
        }
        else
    ...
    _derivativeSpotPriceAfterSwapTokenInForExactTokenOut(poolPairData, amount) {
        if (poolPairData.pairType === PairTypes$1.MainTokenToBpt) {
            return this._derivativeSpotPriceAfterSwapTokenInForExactBPTOut(poolPairData, amount);
        }
        else if (poolPairData.pairType === PairTypes$1.BptToMainToken) {
            return this._derivativeSpotPriceAfterSwapBPTInForExactTokenOut(poolPairData, amount);
        }
        else
...
function _spotPriceAfterSwapTokenInForExactTokenOut$1(amount, poolPairData) {
    ...
    const t = getTimeTillExpiry(poolPairData.expiryTime, poolPairData.currentBlockTimestamp, poolPairData.unitSeconds);
    const Ao = amount.toNumber();
    return bnum(Math.pow((Math.pow(Bi, (1 - t)) + Math.pow(Bo, (1 - t)) - Math.pow((-Ao + Bo), (1 - t))), (-1 + 1 / (1 - t))) /
        Math.pow((-Ao + Bo), t) +
        Math.abs(-1 +
...
// PairType = 'token->token'
// SwapType = 'swapExactIn'
function _derivativeSpotPriceAfterSwapExactTokenInForTokenOut$1(amount, poolPairData) {
    const f = parseFloat(formatFixed(poolPairData.swapFee, 18));
    const Bi = parseFloat(formatFixed(poolPairData.balanceIn, poolPairData.decimalsIn));
    const Bo = parseFloat(formatFixed(poolPairData.balanceOut, poolPairData.decimalsOut));
    const t = getTimeTillExpiry(poolPairData.expiryTime, poolPairData.currentBlockTimestamp, poolPairData.unitSeconds);
    const Ai = amount.toNumber();
    return bnum(-((-((Math.pow((Math.pow(Bi, (1 - t)) - Math.pow((Ai + Bi), (1 - t)) + Math.pow(Bo, (1 - t))), (-2 + 1 / (1 - t))) *
        (-1 + 1 / (1 - t)) *
        (1 - t)) /
        Math.pow((Ai + Bi), (2 * t))) -
        Math.pow((Ai + Bi), (-1 - t)) *
...
// PairType = 'token->token'
// SwapType = 'swapExactOut'
function _derivativeSpotPriceAfterSwapTokenInForExactTokenOut$1(amount, poolPairData) {
    const f = parseFloat(formatFixed(poolPairData.swapFee, 18));
    const Bi = parseFloat(formatFixed(poolPairData.balanceIn, poolPairData.decimalsIn));
    const Bo = parseFloat(formatFixed(poolPairData.balanceOut, poolPairData.decimalsOut));
    const t = getTimeTillExpiry(poolPairData.expiryTime, poolPairData.currentBlockTimestamp, poolPairData.unitSeconds);
    const Ao = amount.toNumber();
    return bnum((Math.pow((Math.pow(Bi, (1 - t)) + Math.pow(Bo, (1 - t)) - Math.pow((-Ao + Bo), (1 - t))), (-2 + 1 / (1 - t))) *
        (-1 + 1 / (1 - t)) *
        (1 - t)) /
        Math.pow((-Ao + Bo), (2 * t)) +
        Math.pow((-Ao + Bo), (-1 - t)) *
...
class ElementPool {
    ...
    getLimitAmountSwap(poolPairData, swapType) {
        ...
        if (swapType === SwapTypes.SwapExactIn) {
            ...
            return bnum(Math.pow((Math.pow(Bi, (1 - t)) + Math.pow(Bo, (1 - t))), (1 / (1 - t))) - Bi);
        }
        else {
            return bnum(formatFixed(poolPairData.balanceOut.mul(MAX_OUT_RATIO).div(WeiPerEther), poolPairData.decimalsOut));
        }
    }
    // Updates the balance of a given token for the pool
    updateTokenBalanceForPool(token, newBalance) {
    ...
    _derivativeSpotPriceAfterSwapExactTokenInForTokenOut(poolPairData, amount) {
        poolPairData.currentBlockTimestamp = this.currentBlockTimestamp;
        return _derivativeSpotPriceAfterSwapExactTokenInForTokenOut$1(amount, poolPairData);
    }
    _derivativeSpotPriceAfterSwapTokenInForExactTokenOut(poolPairData, amount) {
        poolPairData.currentBlockTimestamp = this.currentBlockTimestamp;
        return _derivativeSpotPriceAfterSwapTokenInForExactTokenOut$1(amount, poolPairData);
    }
}
...
function _exactBPTInForTokenOut(amount, poolPairData) {
    ...
    if (amount.isZero())
        return amount;
    const { amp, allBalances, balanceIn, tokenIndexOut, decimalsIn, swapFee } = poolPairData;
    const balances = [...allBalances];
    ...
    **********************************************************************************************/
    ...
    const currentInvariant = _invariant(amp, balances);
    // Calculate new invariant
    const bnumBalanceIn = MAX_TOKEN_BALANCE.minus(bnum(formatFixed(balanceIn, decimalsIn)));
    ...
    const currentWeight = balances[tokenIndexOut].div(sumBalances);
    const tokenBalancePercentageExcess = bnum(1).minus(currentWeight);
    const ans = amountOutBeforeFee.times(ONE$2.minus(tokenBalancePercentageExcess
        .times(swapFee.toString())
        .div(WeiPerEther.toString())));
    return ans;
}
...
class PhantomStablePool {
    ...
    parsePoolPairData(tokenIn, tokenOut) {
        ...
        // VirtualBPTSupply must be used for the maths
        // TO DO - SG should be updated to so that totalShares should return VirtualSupply
        const virtualBptSupply = this.MAX_TOKEN_BALANCE.sub(allBalancesScaled[bptIndex]);
        const poolPairData = {
            id: this.id,
            address: this.address,
            poolType: this.poolType,
    ...
    getNormalizedLiquidity(poolPairData) {
        // This is an approximation as the actual normalized liquidity is a lot more complicated to calculate
        return bnum(formatFixed(poolPairData.balanceOut.mul(poolPairData.amp), poolPairData.decimalsOut + PhantomStablePool.AMP_DECIMALS));
    }
    getLimitAmountSwap(poolPairData, swapType) {
        // PoolPairData is using balances that have already been exchanged so need to convert back
        if (swapType === SwapTypes.SwapExactIn) {
    ...
    _exactTokenInForTokenOut(poolPairData, amount, exact) {
        try {
            ...
            const amtWithFee = this.subtractSwapFeeAmount(BigNumber$1.from(amtScaled.toString()), poolPairData.swapFee);
            ...
            if (poolPairData.pairType === PairTypes.TokenToBpt) {
                const amountsIn = Array(poolPairData.allBalancesScaled.length).fill(ZERO$1);
                amountsIn[poolPairData.tokenIndexIn] = bnum(amountConverted.toString());
                amt = SDK.StableMath._calcBptOutGivenExactTokensIn(bnum(this.amp.toString()), poolPairData.allBalancesScaled.map((b) => bnum(b.toString())), amountsIn, bnum(poolPairData.virtualBptSupply.toString()), ZERO$1 // Fee is handled above
                );
            }
            else if (poolPairData.pairType === PairTypes.BptToToken) {
                amt = SDK.StableMath._calcTokenOutGivenExactBptIn(bnum(this.amp.toString()), poolPairData.allBalancesScaled.map((b) => bnum(b.toString())), poolPairData.tokenIndexOut, bnum(amountConverted.toString()), bnum(poolPairData.virtualBptSupply.toString()), ZERO$1 // Fee is handled above
                );
            }
            else {
                amt = SDK.StableMath._calcOutGivenIn(bnum(this.amp.toString()), poolPairData.allBalancesScaled.map((b) => bnum(b.toString())), poolPairData.tokenIndexIn, poolPairData.tokenIndexOut, bnum(amountConverted.toString()), ZERO$1 // Fee is handled above
                );
            }
            // return normalised amount
            // Using BigNumber.js decimalPlaces (dp), allows us to consider token decimal accuracy correctly,
    ...
    _tokenInForExactTokenOut(poolPairData, amount, exact) {
        try {
            ...
            let amt;
            if (poolPairData.pairType === PairTypes.TokenToBpt) {
                amt = SDK.StableMath._calcTokenInGivenExactBptOut(bnum(this.amp.toString()), poolPairData.allBalancesScaled.map((b) => bnum(b.toString())), poolPairData.tokenIndexIn, bnum(amountConverted.toString()), bnum(poolPairData.virtualBptSupply.toString()), ZERO$1 // Fee is handled above
                );
            }
            else if (poolPairData.pairType === PairTypes.BptToToken) {
                const amountsOut = Array(poolPairData.allBalancesScaled.length).fill(ZERO$1);
                amountsOut[poolPairData.tokenIndexOut] = bnum(amountConverted.toString());
                amt = SDK.StableMath._calcBptInGivenExactTokensOut(bnum(this.amp.toString()), poolPairData.allBalancesScaled.map((b) => bnum(b.toString())), amountsOut, bnum(poolPairData.virtualBptSupply.toString()), ZERO$1 // Fee is handled above
                );
            }
            else {
                amt = SDK.StableMath._calcInGivenOut(bnum(this.amp.toString()), poolPairData.allBalancesScaled.map((b) => bnum(b.toString())), poolPairData.tokenIndexIn, poolPairData.tokenIndexOut, amountConverted, ZERO$1 // Fee is handled above
                );
            }
            const returnScaled = amt
                ...
                .dp(0);
            // In Phantom Pools every time there is a swap (token per token, bpt per token or token per bpt), we substract the fee from the amount in
            const returnWithFee = bnum(this.addSwapFeeAmount(BigNumber$1.from(returnScaled.toString()), poolPairData.swapFee).toString());
            // return human number
    ...
    _derivativeSpotPriceAfterSwapExactTokenInForTokenOut(poolPairData, amount) {
        ...
        if (poolPairData.pairType === PairTypes.TokenToBpt) {
            ...
            result =
                _derivativeSpotPriceAfterSwapExactTokenInForTokenOut(amountConverted, poolPairData);
        }
    ...
    _derivativeSpotPriceAfterSwapTokenInForExactTokenOut(poolPairData, amount) {
        ...
        if (poolPairData.pairType === PairTypes.TokenToBpt) {
            ...
            result =
                _derivativeSpotPriceAfterSwapTokenInForExactTokenOut(amountConverted, poolPairData);
        }
...
function getOutputAmountSwap(pool, poolPairData, swapType, amount) {
    ...
    if (swapType === SwapTypes.SwapExactIn) {
        if (poolPairData.poolType !== PoolTypes.Linear &&
...
function getEffectivePriceSwapForPath(path, swapType, amount, inputDecimals) {
    ...
    const outputAmountSwap = getOutputAmountSwapForPath(path, swapType, amount, inputDecimals);
    if (swapType === SwapTypes.SwapExactIn) {
...
function EVMgetOutputAmountSwap(pool, poolPairData, swapType, amount) {
    ...
    if (swapType === SwapTypes.SwapExactIn) {
        ...
        if (pool.poolType === PoolTypes.Weighted ||
            ...
            returnAmount = pool._exactTokenInForTokenOut(poolPairData, amount, true);
        }
        ...
        if (pool.poolType === PoolTypes.Weighted ||
            ...
            pool.poolType === PoolTypes.MetaStable ||
            pool.poolType === PoolTypes.Linear) {
            returnAmount = pool._tokenInForExactTokenOut(poolPairData, amount, true);
        }
        else if (pool.poolType === PoolTypes.Element) {
            // TODO this will just be part of above once maths available
            returnAmount = getOutputAmountSwap(pool, poolPairData, swapType, amount);
    ...
    pool.updateTokenBalanceForPool(tokenIn, balanceIn.add(parseFixed(returnAmount.dp(poolPairData.decimalsIn).toString(), poolPairData.decimalsIn)));
    pool.updateTokenBalanceForPool(tokenOut, balanceOut.sub(parseFixed(amount.dp(poolPairData.decimalsOut).toString(), poolPairData.decimalsOut)));
    return returnAmount;
}
...
const optimizeSwapAmounts = (paths, swapType, totalSwapAmount, initialSwapAmounts, highestLimitAmounts, inputDecimals, outputDecimals, initialNumPaths, maxPools, costReturnToken) => {
    ...
    let bestTotalReturnConsideringFees = swapType === SwapTypes.SwapExactIn ? INFINITY.times(-1) : INFINITY;
    let bestSwapAmounts = [];
    let bestPaths = [];
    let swapAmounts = initialSwapAmounts.map((amount) => bnum(formatFixed(amount, inputDecimals)));
    for (let b = initialNumPaths; b <= paths.length; b++) {
        if (b != initialNumPaths) {
            ...
            const humanTotalSwapAmount = formatFixed(totalSwapAmount, inputDecimals);
            ...
            swapAmounts.forEach((swapAmount, i) => {
                swapAmounts[i] = swapAmount.times(ONE$2.minus(newSwapAmount.div(humanTotalSwapAmount)));
            });
            swapAmounts.push(newSwapAmount);
        }
        const { paths: selectedPaths, swapAmounts: bestAmounts } = optimizePathDistribution(paths, swapType, totalSwapAmount, swapAmounts, inputDecimals);
        ...
        const costReturnTokenHuman = formatFixed(costReturnToken, outputDecimals);
 ...
 */
const optimizePathDistribution = (allPaths, swapType, totalSwapAmount, initialSwapAmounts, inputDecimals) => {
    let [selectedPaths, exceedingAmounts] = getBestPathIds(allPaths, swapType, initialSwapAmounts, inputDecimals);
    ...
    const humanTotalSwapAmount = bnum(formatFixed(totalSwapAmount, inputDecimals));
    ...
    while (!historyOfSortedPathIds.includes(sortedPathIdsJSON)) {
        ...
        [newSelectedPaths, exceedingAmounts] = getBestPathIds(allPaths, swapType, swapAmounts, inputDecimals);
...
const formatSwaps$1 = (bestPaths, swapType, totalSwapAmount, bestSwapAmounts) => {
    ...
    bestPaths.forEach((path, i) => {
        ...
        */
        ...
        if (swapType === SwapTypes.SwapExactIn) {
            for (let i = 0; i < n; i++) {
                amounts.push(EVMgetOutputAmountSwap(path.pools[i], poolPairData[i], SwapTypes.SwapExactIn, amounts[amounts.length - 1]));
                const swap = {
            ...
            for (let i = 0; i < n; i++) {
                amounts.unshift(EVMgetOutputAmountSwap(path.pools[n - 1 - i], poolPairData[n - 1 - i], SwapTypes.SwapExactOut, amounts[0]));
                const swap = {
    ...
    if (swaps.length > 0) {
        const totalSwapAmountWithRoundingErrors = bestSwapAmounts.reduce((a, b) => a.plus(b), ZERO$1);
    ...
    const marketSp = getSpotPriceAfterSwapForPath(largestSwapPath, swapType, ZERO$1);
...
function getBestPathIds(originalPaths, swapType, swapAmounts, inputDecimals) {
    ...
    sortedSwapAmounts.forEach((swapAmount) => {
        ...
        paths.forEach((path, i) => {
            ...
            if (bnum(formatFixed(path.limitAmount, inputDecimals)).gte(swapAmount)) {
                ...
                let effectivePrice;
                if (bnum(formatFixed(path.limitAmount, inputDecimals)).eq(swapAmount)) {
                    effectivePrice = INFINITY;
                }
                else {
                    // TODO for optimization: pass already calculated limitAmount as input
                    // to getEffectivePriceSwapForPath()
                    effectivePrice = getEffectivePriceSwapForPath(path, swapType, swapAmount, inputDecimals);
                }
        ...
        selectedPathExceedingAmounts.push(swapAmount.minus(bnum(formatFixed(paths[bestPathIndex].limitAmount, inputDecimals))));
...
iterationCount) {
    ...
    swapAmounts.forEach((swapAmount, i) => {
        ...
        if ((iterationCount == 0 &&
            ...
            sumInverseDerivativeSPaSs = sumInverseDerivativeSPaSs.plus(ONE$2.div(derivative_SPaS));
    ...
    while (BigNumber.min.apply(null, swapAmounts).lt(ZERO$1) ||
    ...
    for (let i = 0; i < swapAmounts.length; ++i) {
        if (swapAmounts[i].gt(ZERO$1) && exceedingAmounts[i].lt(ZERO$1)) {
            if (swapAmounts[i].plus(roundingError).gt(ZERO$1) &&
    ...
    swapAmounts.forEach((swapAmount, i) => {
        ...
        if (swapAmount.lte(ZERO$1)) {
            ...
            sumInverseDerivativeSPaSsForNegativeAmounts =
                sumInverseDerivativeSPaSsForNegativeAmounts.plus(ONE$2.div(derivativeSPaSs[i]));
        }
        // Amount is above limit (exceeding > 0)
            ...
            sumInverseDerivativeSPaSsForExceedingAmounts =
                sumInverseDerivativeSPaSsForExceedingAmounts.plus(ONE$2.div(derivativeSPaSs[i]));
        }
        ...
        // i.e. if swapAmount > 0 or swapAmount < limit
            ...
            sumInverseDerivativeSPaSsForViableAmounts =
                sumInverseDerivativeSPaSsForViableAmounts.plus(ONE$2.div(derivativeSPaSs[i]));
    });
...
const getBestPaths = (paths, swapType, totalSwapAmount, inputDecimals, outputDecimals, maxPools, costReturnToken) => {
    ...
    const [swaps, bestTotalReturn, marketSp] = formatSwaps$1(bestPaths, swapType, bnum(formatFixed(totalSwapAmount, inputDecimals)), bestSwapAmounts);
    if (bestTotalReturn.eq(0))
        return [[], ZERO$1, ZERO$1, ZERO$1];
...
function calculateMarketSp(swapType, swaps, assets, pools) {
    ...
    for (let i = 0; i < swaps.length; i++) {
        ...
        let spotPrice;
        if (swapType === SwapTypes.SwapExactIn)
            spotPrice = newPool._spotPriceAfterSwapExactTokenInForTokenOut(poolPairData, ZERO$1);
        // Amount = 0 to just get current SP
        else
            spotPrice = newPool._spotPriceAfterSwapTokenInForExactTokenOut(poolPairData, ZERO$1); // Amount = 0 to just get current SP
...
function getStEthRate(provider, chainId) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const wstETHContract = new Contract(Lido.wstETH[chainId], ['function tokensPerStEth() external view returns (uint256)'], provider);
...
function getWrapperRate(provider, wrapperAddress) {
    return __awaiter(this, void 0, void 0, function* () {
        const ubWrapper = new Contract(wrapperAddress, [
            'function underlyingToWrapper(uint256 amount) external view returns (uint256)',
        ], provider);
        return ubWrapper.underlyingToWrapper(WeiPerEther);
    });
}
...
function getWrappedInfo(provider, swapType, tokenIn, tokenOut, config, swapAmount) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (tokensToUBWrapperMap[tokenIn]) {
            ...
            if (swapType === SwapTypes.SwapExactIn)
                swapAmountForSwaps = swapAmount.mul(tokenInRate).div(WeiPerEther);
        }
        ...
        if (tokensToUBWrapperMap[tokenOut]) {
            ...
            if (swapType === SwapTypes.SwapExactOut)
                swapAmountForSwaps = swapAmount.mul(tokenOutRate).div(WeiPerEther);
        }
        //--------------------------------------------------------------------------
        return {
...
function setWrappedInfo(swapInfo, swapType, wrappedInfo, config) {
    ...
    if (wrappedInfo.tokenIn.wrapType === WrapTypes.ETH ||
        ...
        swapInfo.tokenAddresses = swapInfo.tokenAddresses.map((addr) => isSameAddress(addr, config.weth) ? AddressZero : addr);
    }
    //--------------------------------------------------------------------------
...
const formatSequence = (swapKind, sequence, tokenAddresses) => {
    ...
    return sequence.map((swap, i) => {
        ...
        if (i === 0) {
            ...
            const scalingFactor = swapKind === SwapTypes.SwapExactIn
...
function formatSwaps(swapsOriginal, swapType, swapAmount, tokenIn, tokenOut, returnAmount, returnAmountConsideringFees, marketSp) {
    ...
    if (dust.gt(0)) {
        swaps[0].amount = BigNumber$1.from(swaps[0].amount).add(dust).toString();
    }
    const swapInfo = {
        swapAmount,
        returnAmount,
...
const filterPoolsByType = (pools, poolTypeFilter) => {
    if (poolTypeFilter === PoolFilter.All)
        return pools;
...
function filterHopPools(tokenIn, tokenOut, hopTokens, poolsOfInterest) {
    ...
    for (let i = 0; i < hopTokens.length; i++) {
        ...
        let highestNormalizedLiquiditySecond = ZERO$1; // Aux variable to find pool with most liquidity for pair (hopToken -> tokenOut)
        ...
        if (highestNormalizedLiquidityFirstPoolId &&
            ...
            const path = createPath([tokenIn, hopTokens[i], tokenOut], [
                poolsOfInterest[highestNormalizedLiquidityFirstPoolId],
                poolsOfInterest[highestNormalizedLiquiditySecondPoolId],
            ]);
            paths.push(path);
...
*/
function getLinearStaBal3Paths(tokenIn, tokenOut, poolsAllDict, poolsFilteredDict, config) {
    ...
    if (!staBal3Pool)
        return [];
    if (tokenIn === staBal3PoolInfo.address ||
    ...
    // If both tokenIn and tokenOut belong to linear pools
        ...
        const linearPathway = createPath([tokenIn, linearPoolIn.address, linearPoolOut.address, tokenOut], [linearPoolIn, staBal3Pool, linearPoolOut]);
        ...
        const linearPathway = createPath([tokenIn, linearPoolIn.address, staBal3Pool.address], [linearPoolIn, staBal3Pool]);
        // Creates a path through most liquid staBal3/Token pool
        // staBal3Bpt>[staBal3Bpt-TokenOut]>TokenOut
        const staBal3TokenOutPath = getMostLiquidPath([staBal3Pool.address, tokenOut], 0, [], poolsFilteredDict, SwapPairType.HopOut);
        ...
        if (!wethStaBal3Pool)
            return pathsUsingLinear;
        const wethTokenOutPath = getMostLiquidPath([staBal3Pool.address, config.weth, tokenOut], 1, [wethStaBal3Pool], poolsFilteredDict, SwapPairType.HopOut);
        ...
        const linearPathway = createPath([staBal3Pool.address, linearPoolOut.address, tokenOut], [staBal3Pool, linearPoolOut]);
        // Creates a path through most liquid staBal3/Token pool
        // TokenIn>[TokenIn-staBal3Bpt]>staBal3Bpt
        const tokenInStaBal3Path = getMostLiquidPath([tokenIn, staBal3Pool.address], 0, [], poolsFilteredDict, SwapPairType.HopIn);
        ...
        if (!wethStaBal3Pool)
            return pathsUsingLinear;
        const tokenInWethPath = getMostLiquidPath([tokenIn, config.weth, staBal3Pool.address], 0, [wethStaBal3Pool], poolsFilteredDict, SwapPairType.HopIn);
...
*/
function getMostLiquidPath(tokens, tokenInIndex, pathPools, pools, swapPairType) {
    const tokenIn = tokens[tokenInIndex];
    const tokenOut = tokens[tokenInIndex + 1];
    // Finds pool with highest liquidity for tokenIn/Out
    const mostLiquidPool = getHighestLiquidityPool(tokenIn.toLowerCase(), tokenOut.toLowerCase(), swapPairType, pools);
    // If there is a paired pool create a path
    if (mostLiquidPool === null)
        return {};
...
function getHighestLiquidityPool(tokenIn, tokenOut, swapPairType, poolsOfInterest) {
    let highestNormalizedLiquidity = ZERO$1;
    let highestNormalizedLiquidityPoolId = null;
    for (const id in poolsOfInterest) {
        const pool = poolsOfInterest[id];
        if (swapPairType != pool.swapPairType)
            continue;
        const tokenListSet = new Set(pool.tokensList);
...
function getPathsUsingStaBalPool(tokenIn, tokenOut, poolsAll, poolsFiltered, config) {
    ...
    const metastablePoolIdIn = getHighestLiquidityPool(tokenIn, hopTokenStaBal, SwapPairType.HopIn, poolsFiltered);
    // Finds the best metastable Pool with tokenOut/staBal3Bpt or returns null if doesn't exist
    const metastablePoolIdOut = getHighestLiquidityPool(hopTokenStaBal, tokenOut, SwapPairType.HopOut, poolsFiltered);
    if (metastablePoolIdIn && !metastablePoolIdOut) {
        ...
        const mostLiquidLastPool = getHighestLiquidityPool(usdcConnectingPoolInfo.usdc, tokenOut, SwapPairType.HopOut, poolsFiltered);
    ...
    if (!metastablePoolIdIn && metastablePoolIdOut) {
        ...
        const mostLiquidFirstPool = getHighestLiquidityPool(tokenIn, usdcConnectingPoolInfo.usdc, SwapPairType.HopIn, poolsFiltered);
...
function getLimitAmountSwapForPath(path, swapType) {
    ...
    let limit;
    if (swapType === SwapTypes.SwapExactIn) {
        limit = path.pools[poolPairData.length - 1].getLimitAmountSwap(poolPairData[poolPairData.length - 1], SwapTypes.SwapExactIn);
        for (let i = poolPairData.length - 2; i >= 0; i--) {
            const poolLimitExactIn = path.pools[i].getLimitAmountSwap(poolPairData[i], SwapTypes.SwapExactIn);
            const poolLimitExactOut = path.pools[i].getLimitAmountSwap(poolPairData[i], SwapTypes.SwapExactOut);
            if (poolLimitExactOut.lte(limit)) {
                limit = poolLimitExactIn;
            }
            else {
                const pulledLimit = getOutputAmountSwap(path.pools[i], path.poolPairData[i], SwapTypes.SwapExactOut, limit);
                limit = BigNumber.min(pulledLimit, poolLimitExactIn);
            }
        }
        if (limit.isZero())
            return Zero;
        const result = parseFixed(limit.dp(poolPairData[0].decimalsIn).toString(), poolPairData[0].decimalsIn);
        return result;
    }
    else {
        limit = path.pools[0].getLimitAmountSwap(poolPairData[0], SwapTypes.SwapExactOut);
        for (let i = 1; i < poolPairData.length; i++) {
            ...
            if (poolLimitExactIn.lte(limit)) {
                ...
                const pushedLimit = getOutputAmountSwap(path.pools[i], path.poolPairData[i], SwapTypes.SwapExactIn, limit);
                limit = BigNumber.min(pushedLimit, poolLimitExactOut);
            }
        }
        if (limit.isZero())
            return Zero;
        return parseFixed(limit
...
class RouteProposer {
    ...
    getCandidatePaths(tokenIn, tokenOut, swapType, pools, swapOptions) {
        ...
        const [poolsFilteredDict, hopTokens] = filterPoolsOfInterest(poolsAllDict, tokenIn, tokenOut, swapOptions.maxPools);
        const [, pathData] = filterHopPools(tokenIn, tokenOut, hopTokens, poolsFilteredDict);
        const pathsUsingLinear = getLinearStaBal3Paths(tokenIn, tokenOut, poolsAllDict, poolsFilteredDict, this.config);
        const pathsUsingStaBal = getPathsUsingStaBalPool(tokenIn, tokenOut, poolsAllDict, poolsFilteredDict, this.config);
...
class SwapCostCalculator {
     ...
     */
    convertGasCostToToken(tokenAddress, tokenDecimals, gasPriceWei, swapGas = BigNumber$1.from('35000')) {
        return __awaiter(this, void 0, void 0, function* () {
            if (gasPriceWei.isZero() || swapGas.isZero())
                return Zero;
            const tokenPrice = yield this.getNativeAssetPriceInToken(tokenAddress);
            const tokenPriceWei = BigNumber$1.from(scale(bnum(tokenPrice), tokenDecimals).dp(0).toString());
...
class SOR {
    ...
    getSwaps(tokenIn, tokenOut, swapType, swapAmount, swapOptions) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const wrappedInfo = yield getWrappedInfo(this.provider, swapType, tokenIn, tokenOut, this.config, BigNumber$1.from(swapAmount));
            let swapInfo;
    ...
    processSwaps(tokenIn, tokenOut, swapType, swapAmount, pools, swapOptions) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const costOutputToken = yield this.getCostOfSwapInToken(swapType === SwapTypes.SwapExactIn ? tokenOut : tokenIn, swapType === SwapTypes.SwapExactIn
                ? tokenOutDecimals
    ...
    getBestPaths(paths, swapAmount, swapType, tokenInDecimals, tokenOutDecimals, costOutputToken, maxPools) {
        ...
        const [inputDecimals, outputDecimals] = swapType === SwapTypes.SwapExactIn
        ...
        return [
            swaps,
            parseFixed(total.dp(outputDecimals, BigNumber.ROUND_FLOOR).toString(), outputDecimals),
            marketSp.toString(),
            parseFixed(totalConsideringFees
...
function BPTForTokensZeroPriceImpact$2(balances, decimals, normalizedWeights, amounts, bptTotalSupply) {
    const amountBPTOut = amounts.reduce((totalBptOut, amountIn, i) => {
        ...
        const BPTPrice = _spotPriceAfterSwapTokenInForExactBPTOut$3(ZERO$1, poolPairData);
        ...
        const downscaledBptOut = bnum(downscaledAmountIn)
            .div(BPTPrice)
            .toString();
        return BigNumber$1.from(totalBptOut).add(parseFixed(downscaledBptOut, 18));
    }, Zero);
...
bptTotalSupply, amp) {
    ...
    const allBalancesDownScaled = allBalances.map((balance, i) => bnum(formatFixed(balance, decimals[i])));
    const amountBPTOut = amounts.reduce((totalBptOut, amountIn, i) => {
        // Calculate amount of BPT gained per token in
        const poolPairData = {
            amp: amp,
        ...
        const BPTPrice = _spotPriceAfterSwapTokenInForExactBPTOut$2(ZERO$1, poolPairData);
        // Multiply by amountIn to get contribution to total bpt out
        const downscaledAmountIn = formatFixed(amountIn, decimals[i]);
        const downscaledBptOut = bnum(downscaledAmountIn)
            .div(BPTPrice)
...
function queryBatchSwapTokensIn(sor, vaultContract, tokensIn, amountsIn, tokenOut) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (let i = 0; i < tokensIn.length; i++) {
            const swap = yield getSorSwapInfo(tokensIn[i], tokenOut, SwapTypes.SwapExactIn, amountsIn[i].toString(), sor);
        ...
        try {
            // Onchain query
            const deltas = yield queryBatchSwap(vaultContract, SwapTypes.SwapExactIn, batchedSwaps.swaps, batchedSwaps.assets);
            amountTokenOut =
...
function queryBatchSwapTokensOut(sor, vaultContract, tokenIn, amountsIn, tokensOut) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (let i = 0; i < tokensOut.length; i++) {
            const swap = yield getSorSwapInfo(tokenIn, tokensOut[i], SwapTypes.SwapExactIn, amountsIn[i].toString(), sor);
            swaps.push(swap.swaps);
            assetArray.push(swap.tokenAddresses);
        }
        // Join swaps and assets together correctly
...
export { PoolFilter, PoolTypes, SOR, SwapPairType, SwapTypes, parseToPoolsDict, BPTForTokensZeroPriceImpact as phantomStableBPTForTokensZeroPriceImpact, queryBatchSwapTokensIn, queryBatchSwapTokensOut, BPTForTokensZeroPriceImpact$1 as stableBPTForTokensZeroPriceImpact, BPTForTokensZeroPriceImpact$2 as weightedBPTForTokensZeroPriceImpact };
